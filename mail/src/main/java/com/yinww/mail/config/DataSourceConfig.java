...
public class DataSourceConfig {
    ...
    public DataSource getDataSource() throws SQLException{
        ...
        tableRuleConfig.setDatabaseShardingStrategyConfig(new InlineShardingStrategyConfiguration(databaseShardingColumn, databaseAlgorithmExpression.replace("->", "")));
        ...
        // 获取数据源对象
        return ShardingDataSourceFactory.createDataSource(getDataSourceMap(), shardingRuleConfig, new ConcurrentHashMap(), new Properties());
    }
	...
	private DataSource buildDataSource(String name, Properties properties) {
		try {
			...
			DataSource dataSource = DataSourceBuilder.create().type(typeClass).driverClassName(driverClassName)
